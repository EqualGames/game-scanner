...
pub fn read(file: &str, launcher_path: &String) -> std::io::Result<Game> {
  ...
  let mut launch_command: String = String::from(launcher_path);
  launch_command.push_str(" origin2://game/launch?offerIds=");
  launch_command.push_str(&manifest.id);

  let name = std::path::Path::new(&file.to_string()).parent().unwrap().file_name().unwrap().to_str().unwrap().to_string();
